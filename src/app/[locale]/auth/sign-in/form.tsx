...
const SignInForm: React.FC<SignInFormProps> = ({ className }) => {
  ...
  const onSubmit = form.handleSubmit(async (data) => {
    try {
      ...
      if (!signIn.data?.user.emailVerified) {
    ...
    <form onSubmit={onSubmit} className={cn('p-6 md:p-8', className)}>
      <div className="flex flex-col gap-6">
        <div className="flex flex-col items-center text-center">
          ...
          <p className="text-balance text-slate-500">{t('signIn.subWelcome')}</p>
        </div>
        ...
        <div className="grid gap-2">
          <div className="flex items-center">
            ...
            <Link href={Route.AuthPasswordForgot} className="ml-auto text-sm underline-offset-2 hover:underline">
        ...
        <div className="after:border-border relative text-center text-sm after:absolute after:inset-0 after:top-1/2 after:z-0 after:flex after:items-center after:border-t">
          <span className="relative z-10 bg-white px-2 text-slate-500">{t('signIn.form.orContinueWith')}</span>
        </div>
        <div className="grid grid-cols-3 gap-4">
          <Button type="button" variant="outline" className="w-full" disabled={form.formState.isSubmitting}>
            ...
            <span className="sr-only">{t('signIn.form.loginWith.apple')}</span>
          </Button>
          <Button type="button" variant="outline" className="w-full" disabled={form.formState.isSubmitting}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <path
            ...
            <span className="sr-only">{t('signIn.form.loginWith.google')}</span>
          </Button>
          <Button type="button" variant="outline" className="w-full" disabled={form.formState.isSubmitting}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <path
            ...
            <span className="sr-only">{t('signIn.form.loginWith.meta')}</span>
          </Button>
        </div>
        <div className="text-center text-sm">
          {t('signIn.dontHaveAnAccount')}{' '}
          <Link href={Route.AuthSignUp} className="underline underline-offset-4">
            {t('signIn.signUpLink')}
          </Link>
        </div>
      </div>
