...
const SignUpForm: React.FC<SignUpFormProps> = ({ className }) => {
  ...
  const onSubmit = form.handleSubmit(async (data) => {
    try {
      ...
      if (signedUp.data?.user.id) {
        router.push(`${Route.AuthVerifyEmail}?email=${data.email}`)
        return
      }

      if (signedUp.error?.message) {
        toast.error(t('somethingWentWrongError.title'), {
          description: signedUp.error.message,
        })
      }
    } catch (error) {
      toast.error(t('somethingWentWrongError.title'), {
    ...
    <form onSubmit={onSubmit} className={cn('p-6 md:p-8', className)}>
      <div className="flex flex-col gap-6">
        <div className="flex flex-col items-center text-center">
          <h1 className="text-2xl font-bold">{t('signUp.welcome')}</h1>
          <p className="text-balance text-slate-500">{t('signUp.subWelcome')}</p>
        </div>
        <div className="grid gap-2">
          ...
          <Input
            ...
            required
          />
          {form.formState.errors.name && <FormElementMessage>{form.formState.errors.name.message}</FormElementMessage>}
        </div>
        <div className="grid gap-2">
        ...
        <div className="after:border-border relative text-center text-sm after:absolute after:inset-0 after:top-1/2 after:z-0 after:flex after:items-center after:border-t">
          <span className="relative z-10 bg-white px-2 text-slate-500">{t('signUp.form.orContinueWith')}</span>
        </div>
        <div className="grid grid-cols-3 gap-4">
          <Button type="button" variant="outline" className="w-full" disabled={form.formState.isSubmitting}>
            ...
            <span className="sr-only">{t('signUp.form.loginWith.apple')}</span>
          </Button>
          <Button type="button" variant="outline" className="w-full" disabled={form.formState.isSubmitting}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <path
            ...
            <span className="sr-only">{t('signUp.form.loginWith.google')}</span>
          </Button>
          <Button type="button" variant="outline" className="w-full" disabled={form.formState.isSubmitting}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <path
            ...
            <span className="sr-only">{t('signUp.form.loginWith.meta')}</span>
          </Button>
        </div>
